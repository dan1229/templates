...
#
# **
# NOTE: you MUST have cloc installed for this to work
# e.g. brew install cloc
# **
#
...
if [[ "$CURR_DIR" == *"$SCRIPT_DIR"* ]]
then
  cd ..
  CURR_DIR=${CURR_DIR/$SCRIPT_DIR/}  # remove 'scripts' from curr_dir
fi
...
SUMMARY="$(cloc "${CURR_DIR}" --include-lang="JavaScript" --md | tail -1)"
...
DUMB_COMMENTS="$(grep -r -E '//////|// -----' "${CURR_DIR}" | wc -l)"
